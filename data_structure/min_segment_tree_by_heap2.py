...
class MinSegTree:
    ...
    def build(self, nums, l, r, i):
        ...
        if l == r: # If there is only one element
            ...
            return l
        mid = (l + r) // 2
        li = self.build(nums, l, mid, i*2+1)
        ri = self.build(nums, mid+1, r, i*2+2)
        self.nodes[i] = li if self.nums[li] < self.nums[ri] else ri
        return self.nodes[i]
    ...
    def _min(self, ql, qr, l, r, i):
        # print(l, r, i)
        if ql <= l and qr >= r:
            return self.nums[self.nodes[i]] if self.nodes[i] > 0 else MinSegTree.NO_VALUE
        if ql > r or qr < l:
            return MinSegTree.NO_VALUE
        mid = (l + r) // 2
        return min(self._min(ql, qr, l, mid, i*2+1), self._min(ql, qr, mid+1, r, i*2+2))

st = MinSegTree([1,2,3,4,3,2,1])
print(st.nodes)
print(st.query_min(4, 5))
