...
class MinSegTree:
    ...
    def __init__(self, nums):
        ...
        self.nodes = [(MinSegTree.NO_VALUE, -1)] * self.max_size
...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        ...
        st = MinSegTree(heights)
        return self.max_area(st, heights, 0, len(heights)-1)

    def max_area(self, st, heights, l, r):
        if l > r: return -1
        if l == r: return heights[l]
        h, m = st.query_min(l, r)
        return max(self.max_area(st, heights, l, m-1), \
                self.max_area(st, heights, m+1, r), \
                (r-l+1) * h)
